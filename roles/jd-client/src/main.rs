...
use lib::{
    error::{Error as LibError, ProxyResult},
...
use roles_logic_sv2::utils::Mutex as RolesMutex;
use std::{
...
fn process_cli_args<'a>() -> ProxyResult<'a, ProxyConfig> {
    let args = match Args::from_args() {
        ...
        Err(help) => {
            error!("{}", help);
            return Err(LibError::BadCliArgs);
        }
    };
...
async fn main() {
    ...
    let task_collector = Arc::new(RolesMutex::new(vec![]));
...
async fn initialize_jd_as_solo_miner(
    ...
    task_collector: Arc<RolesMutex<Vec<AbortHandle>>>,
    timeout: Duration,
) {
    ...
    TemplateRx::connect(
        ...
        downstream,
        task_collector,
        Arc::new(RolesMutex::new(PoolChangerTrigger::new(timeout))),
        miner_tx_out.clone(),
...
lazy_static! {
    static ref IF_DECRYPTION_PASSWORD_SET: std::sync::Mutex<bool> = std::sync::Mutex::new(false);
    static ref DECRYPTION_PASSWORD: std::sync::Mutex<String> = std::sync::Mutex::new(String::new());
}

fn get_decryption_password() -> Result<String, Box<dyn StdError>> {
    let mut if_set = IF_DECRYPTION_PASSWORD_SET.lock().unwrap();
    if *if_set {
        return Ok(DECRYPTION_PASSWORD.lock().unwrap().clone())
    }
    if let Ok(password) = env::var("DECRYPTION_PASSWORD") {
        *DECRYPTION_PASSWORD.lock().unwrap() = password;
        *if_set = true;
    } else {
        print!("Enter your encrypt password: ");
        let password = read_password()?;
        *DECRYPTION_PASSWORD.lock().unwrap() = password.trim().to_string();
        *if_set = true;
    }
    Ok(DECRYPTION_PASSWORD.lock().unwrap().clone())
}
...
async fn initialize_jd(
    ...
    let decryption_password = get_decryption_password().expect("Failed to get decryption password");
    ...
    let upstream = match lib::upstream_sv2::Upstream::new(
        ...
        upstream_config.tbc_encrypted_kyc_privkey.clone(),
        decryption_password.clone(),
        upstream_config.tbc_miner_kyc_sig.clone(),
        upstream_config.charge_addr.clone(),
